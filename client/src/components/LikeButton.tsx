  ...
  return (
    <>
      <div className={kind === 'post' ? '_feed_like_control' : '_comment_like_control'}>
        <button type="button" className={kind === 'post' ? `_feed_inner_timeline_reaction_emoji${liked ? ' _feed_reaction_active' : ''}` : '_comment_reply_link'} onClick={toggle} aria-pressed={liked} disabled={busy}>{kind === 'post' && <LikeIcon />}<span>{liked ? 'Liked' : 'Like'}</span></button>
