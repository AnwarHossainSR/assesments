...
authRoutes.post('/register', authRateLimit, validate(registerSchema), async (req, res, next) => {
  try { const user = await registerUser(req.body); setAuthCookie(res, signToken(user.id)); res.status(201).json({ user }); }
  catch (err) { next(err); }
});

authRoutes.post('/login', authRateLimit, validate(loginSchema), async (req, res, next) => {
